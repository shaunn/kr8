...
var (
	...
	longcolor   string = "\033[1m\033[33m" /* Bold Yellow */
	longnocolor string = "\033[0m"         // color reset
)
...
func logInit() {
	...
	if long {
		...
		log.Logger = log.Output(zerolog.ConsoleWriter{Out: os.Stderr, NoColor: !viper.GetBool("color"),
	...
	case (trace) || (logLevel == "trace"):
		infolog(nil).Msg("Log level set to TRACE. Enjoy the firehose!")
		zerolog.SetGlobalLevel(zerolog.TraceLevel)
		trace = true
	case (debug) || (logLevel == "debug") || (os.Getenv("GODEBUG") == "true"):
		debug = true
		zerolog.SetGlobalLevel(zerolog.DebugLevel)
		infolog(nil).Msg("Log level set to DEBUG. Hope you find what you are looking for!")
	case (warn) || (logLevel == "warn"):
		warn = true
		infolog(nil).Msg("Log level set to WARN.")
		zerolog.SetGlobalLevel(zerolog.WarnLevel)
	case logLevel == "error":
		infolog(nil).Msg("Log level set to ERROR.")
		zerolog.SetGlobalLevel(zerolog.ErrorLevel)
	case logLevel == "fatal":
		infolog(nil).Msg("Log level set to FATAL.")
		zerolog.SetGlobalLevel(zerolog.FatalLevel)
	case logLevel == "panic":
		infolog(nil).Msg("Log level set to PANIC.")
